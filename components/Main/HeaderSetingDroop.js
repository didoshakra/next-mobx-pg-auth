...
import IconSeting from "../../raui/svg/head/IconSetting_border"
import IconMoon from "../../raui/svg/head/IconMoon_border"
import IconSun from "../../raui/svg/head/IconSun_border"
    ...
    <div ref={wRef_HeaderSetingDroop} className="headerSetingDroop">
      ...
      <ul className="headerSetingDroop__dropdown">
        ...
        <li className="headerSetingDroop__dropdown__item" onClick={() => setUserMenuOpen(!userMenuOpen)}>
          ...
          <p>
            {global.isAuth ? (
              <Image style={{ borderRadius: "50%" }} width={30} height={30} src="/avatar/2.jpg" alt="avatar" />
            ) : (
              <NotAuthenticated
                width={theme.size.headIcon}
                height={theme.size.headIcon}
          ...
          <p>Профіль</p>
          {/* Випадаюче меню User */}
          {userMenuOpen ? <UserMenuDroop userMenuOpen={userMenuOpen} setUserMenuOpen={setUserMenuOpen} /> : ""}
        </li>
      </ul>
      <style jsx global>{`
        .headerSetingDroop {
