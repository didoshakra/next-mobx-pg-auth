...
const MobileMenuDroop = ({ menu,mobileMenuOpen, setMobileMenuOpen }) => {
  ...
  useEffect(() => {
    const onClick = (e) => wRef_MobileMenuDroop.current.contains(e.target) || setMobileMenuOpen(false)
    ...
    return menu.map((item, index) => {
      return (
        <li className="mobileMenuDroop__dropdown__item" key={index}>
          <Link className="mobileMenuDroop__dropdown__item" href={`${item.link}`}>
            {item.a}
          </Link>
          <style jsx>{`
